...
> **Disabled in the Work brain.** `$ingest` must stop at the domain gate and
> ask for an approved private destination. Do not execute the output steps
> below under `Work/`; they are retained only as migration reference.
