...
- **Stable, reusable knowledge** → update the canonical owner identified by the
  dispatcher's project-ownership gate. A project bridge is updated only when it
  owns the fact or needs a thin status/link pointer.
...
- **Action items embedded in the source** → apply the `$create-task` contract,
  set `source: ingest`, and link back.

## Decision rubric

| If the source is... | Default destination |
|---|---|
| Strategy / planning doc | Canonical project owner (`PROJECT.md`, ADR, or equivalent) + artifact snapshot if dense |
...
| Personal or mixed notes | Stop at the domain gate; do not write to the Work brain |
