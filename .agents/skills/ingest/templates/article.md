...
The most important thing is that entities with durable work relevance land in
the right `agent_brain/references/` hubs with concise, evidence-based
descriptions. Passing mentions do not earn entries.

For each qualifying entity:

1. Find the relevant hub. Add an entry:
   ```
   - **<Name>** (<provider/origin>, <YYYY-MM>) — <one-sentence evidence-based description> · [[link to source artifact or raw]]
   ```
...
Body:
- **TL;DR** (3–5 evidence-led bullets; distinguish the source's claims from
  the user's explicit views)
