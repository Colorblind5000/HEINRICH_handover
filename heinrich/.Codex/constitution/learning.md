...
- If a durable fact is user-specific, store it in `agent_brain/`, not in `.Codex/constitution/`.
...
- Learning happens inline during work, subject to the authority and privacy
  gates in `learning-aggressiveness.md`.
- No active batch consolidator is assumed. The skill performing a write must
  complete domain, ownership, duplicate, conflict, and provenance checks.
