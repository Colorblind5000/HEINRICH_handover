...
The CLI indexes the **whole vault**, including `.Codex/`, `artifacts/`, and any `workspace/` sibling — not just `agent_brain/`. Use it when available; fall back to grep + Read otherwise.

## Default order

1. Read the relevant `.Codex/constitution/` docs if the task depends on operating rules.
...
- `agent_brain/tasks/` plus registered satellite `tasks/` — the execution
  layer. Use the bridge to resolve project task paths.
...
If a question can be answered well from `agent_brain/` plus the relevant `.Codex/constitution/` docs, don't go elsewhere. Use search to figure out whether the answer is there.
