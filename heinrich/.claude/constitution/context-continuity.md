...
|---|---|---|
| **Hard — compaction** | Context window fills; harness flattens to a summary, losing detail | The full triad below: `/learn` (durable knowledge) + `/handover` (in-flight task state) **before** `/compact`, then `/resume-handover` next session |
...
`/learn` writes under time pressure (pre-compact, conversation-scoped) and marks new pages `state: needs-review`. Later review is always explicit: surface overlaps and thin pages, but never auto-promote or auto-merge them. `/learn` provenance is load-bearing.
